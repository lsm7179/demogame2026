...
## v0.9.2 오염 Echo 및 캐시 회귀

- PASS: Stage 4의 세 구역에서 오염 Echo 로컬 상대 경로와 벽 충돌 이동 실행
- PASS: 두 번의 루프 전환 후 동일한 결정적 변환과 발사 이벤트 처리 유지
- PASS: CSS와 JavaScript 26개가 모두 `v=0.9.2` 캐시 키로 로드됨
- PASS: Node 자동 테스트 113개, 포맷·문법·릴리스 버전 검사
- PASS: 브라우저 콘솔 오류 없음

## v0.9.0 캠페인 출시 후보 회귀

- PASS: Stage 1~5 연속 월드 데이터, 구역 웨이브와 목표 구성 자동 검증
- PASS: 오염 Echo의 시간 보간, 2초 지연, 발사 단회 처리와 저프레임 제한
- PASS: PRIME Weaver의 동시 사격 방벽과 3단계 패턴 데이터
- PASS: 절차적 음악 장면, 볼륨 정규화와 기존 음소거 저장 호환성
- PASS: Stage 5 승리 전용 엔딩과 캠페인 통계 계산
- PASS: 터치·게임패드 스틱 데드존, 대시·일시정지·메뉴 입력 계산
- PASS: Node 자동 테스트 98개, 포맷·문법·배포 범위·메타데이터 검사
- PASS: Chrome에서 Stage 2~5 구역, 최종보스, 오디오 설정, 엔딩 화면 부팅
- PASS: `signal-keeper/` 변경 없음

실제로 확인하지 않아 PASS로 기록하지 않은 항목:

- Stage 1~5를 처음부터 끝까지 플레이한 완주성 및 10회 장비별 플레이 통계
- 물리 터치 기기와 실제 게임패드 입력
- Safari·Firefox 실기기와 GitHub Pages 배포본
- 30분 연속 실행의 메모리 증가와 장기 프레임 안정성
...
- PASS: 1280×720에서 전투 HUD 여섯 영역의 DOM 경계 겹침 없음
- PASS: 타이틀 → 구역 선택 → Stage 1 브리핑 → 전투를 실제 브라우저에서 확인
- PASS: 로컬 전용 미리보기로 장비 선택과 강화 선택을 실제 게임 코드로 렌더링
- PASS: 장비·강화 카드의 기본 설명이 두 줄 제한 안에서 표시됨
- PASS: 스테이지·무기·방어구·유물 아이콘이 이모지 없는 동일 SVG 선형 체계를 사용
- PASS: 전투, 저장 스키마와 밸런스 상수 변경 없음
- NOT RUN: Stage 1~3 전체 완주와 실제 보상 획득 흐름
- NOT RUN: Safari·Firefox 및 별도 1440×900/1920×1080 실기기 창 검증
