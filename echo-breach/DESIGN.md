...
루프·시간·Echo는 상단 중앙, 체력·방어막·대시는 좌측 하단, 세 장비 슬롯은 하단 중앙, Overdrive는 우측 하단에 고정한다. 우측 상단 전술 미니맵은 플레이어를 주황 마름모, Echo를 청록 링, 적을 적색 점, 릴레이를 보라/청록 링, Anchor를 적색 마름모로 표시한다. 목표 문구는 장문의 설명 대신 `ADVANCE`, `ECHO SWITCH`, `SYNC RELAYS`, `ANCHOR EXPOSED` 상황으로 제한한다.

UI는 먹색·짙은 남색·탁한 금속을 기본 재질로 사용하고 청록과 탁한 적색은 시간 에너지와 위험 상태에만 쓴다. 반복 유리 패널과 상시 네온 발광 대신 단단한 모서리, 마모 흔적, 글꼴 크기와 여백으로 계층을 만든다. 무기 슬롯은 긴 총기 프레임, 방어구는 각진 장갑 프레임, 시간 유물은 원형 프레임으로 구분하며 모든 아이콘은 동일한 선 굵기의 SVG 경로를 사용한다.

환경은 고정 좌표 기반 바닥 오염·균열과 벽 그림자/상단 테두리로 깊이를 만들며 충돌 판정에는 영향을 주지 않는다. Echo 합류, 릴레이 동기화, Overdrive, Anchor 단계 변화와 파괴에만 짧은 시간 왜곡을 사용한다. `prefers-reduced-motion`에서는 카메라 흔들림과 왜곡 강도를 낮춘다.

## 선택 문구 규칙

기본 언어는 한국어다. 업그레이드와 장비의 기본 설명은 효과와 핵심 수치를 한 문장, 최대 두 줄로 표시한다. 장비의 세부 수치·장점·단점은 별도 행으로 분리한다. 화면 코드는 ID만 전달하고 `copy-data.js`가 언어별 이름과 문구를 제공하며, 누락 시 짧은 한국어 기본 문구를 사용한다.
...
기존 Echo와 오염 Echo를 전투 중 즉시 구분할 수 있는지, Stage 2 장벽과 Stage 3 호위가 진행을 막지 않는지, Stage 5의 세 릴레이가 실제로 Echo 역할 분담을 요구하는지 확인한다. 터치·게임패드 실기기와 세 브라우저, 30분 연속 실행도 출시 판정에 포함한다.
